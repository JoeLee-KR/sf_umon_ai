...
import StorageChart, { RangeOption } from '@/components/storage/StorageChart';
import StorageTable from '@/components/storage/StorageTable';
import { StorageUsageResponse } from '@/types/storage';

export default function StoragePage() {
  const [response, setResponse] = useState<StorageUsageResponse | null>(null);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-zinc-900">스토리지 사용량 조회</h1>
          </div>
          <p className="text-xs text-zinc-500 mt-1">
            Snowflake 스토리지 사용량 기록(sf_storage_usage) 시계열 트렌드 및 원천 데이터 조회
          </p>
        </div>
        ...
        <div className="flex items-center justify-between pb-3 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-zinc-800">
              스토리지 사용량 추이 (Storage Usage Trend)
            </h2>
          </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-5 shadow-2xs space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-zinc-800">
              스토리지 원천 데이터 (sf_storage_usage Raw Data)
            </h2>
          </div>
          <div className="flex items-center gap-1.5 text-xs text-zinc-500">
            <Database className="h-3.5 w-3.5 text-zinc-400" />
            <span>sf_storage_usage</span>
          </div>
        </div>
