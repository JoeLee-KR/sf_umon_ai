      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (errJson.db_host || errJson.db_name || errJson.db_user) {
            errorMessage = `mysql 연결할 수 없음 (${errJson.db_host || ""}, ${errJson.db_name || ""}, ${errJson.db_user || ""})`;
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
          ...
          {response && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              {(response.db_host || response.db_name || response.db_user) && (
                <div className="inline-flex items-center gap-2 px-2 py-0.5 rounded-md text-[10px] font-mono border bg-zinc-50 text-zinc-600 border-zinc-200">
                  {response.db_host && (
                    <span className="flex items-center gap-1">

                      <span className="text-zinc-700 font-medium">{response.db_host}</span>
                    </span>
                  )}
                  ...
                  {response.db_name && (
                    <span className="flex items-center gap-1">
                      ...
                      <span className="text-zinc-700 font-medium">{response.db_name}</span>
                    </span>
                  )}
                  ...
                  {response.db_user && (
                    <span className="flex items-center gap-1">
                      <span className="text-zinc-400 font-sans">:</span>
                      <span className="text-zinc-700 font-medium">{response.db_user}</span>
                    </span>
                  )}
                </div>
              )}
